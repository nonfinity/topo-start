...
/* ****************************************************************** */
  ...
  export class solidCore {
    // declare some shit
      ...
      protected planes = [];              // array of planes in this solid

    // CONSTRUCTOR. yay!
      constructor(market: object, envelope: object) {
        this.market = market;
        this.envelope = envelope; }
    
    // Building up a solid
      populateFromEnvelope() {
        ...
        for(let i of this.planes) { i.addLinesBetween(bDate, eDate) }
      }

      addPlanes(account: object | object[]) {
        if (Array.isArray(account)) { 
          for (let i of account) { this.planes.push(new planeCore(this.market, this.envelope, i)) }
        } else {
          this.planes.push( new planeCore(this.market, this.envelope, account) )
        }
      }
    ...
    // Accessing / retreiving members
      getPlane(index: number): planeCore {
    ...
    // Summarize Totals
      public aggregate(childPath: string[], parentPath: string[] = childPath): void {
        ...
        for (let i of this.planes) { i.aggregate(childPath, parentPath) }
        // ok. now proceed
        
        let q = this.planes[0].getCalc(childPath)
        ...
        for (let i of Object.keys(q)) {
          ...
          if (typeof q[i] == 'object') {  // if the element is an object, iterate down through it
            ...
            let tempVal = 0;
            for(let j of this.planes) { tempVal = tempVal + j.getCalc([...childPath, i]) }
            this.setCalc([...parentPath, i], tempVal)
          }
        } }

    // Push down a change in revenue
      public pushRevRate(newRate: number): void {
        for (let i of this.planes) { i.pushRevRate(newRate) }
