...
/*  Description
 *
 * The purpose of this version is to reframe from the use case of adding/removing many hypers from a fold.
 * Core piece is to begin using promises
 * 
*/
// VERSION INFORMATION

export interface objSet {  [name: string] : any; }
...
export abstract class absCore {
  ...
  constructor(dataItems: objSet, heritage: objSet) {
    if (heritage.parentObj != undefined) { this.setHeritage(heritage.parentObj, heritage.parentName) }
    for(let i of Object.keys(dataItems)) { this.setData([i],dataItems[i]) }
    this.setData(['version','absCore'], vNum)
  ...
  protected setHeritage(parent: absCore, parentName: string) {
    for(let i of Object.keys(parent.heritage)) {
      this.heritage[i] = parent.heritage[i]
    }
    this.heritage[parentName] = parent;
  }
  ...
  // members which will need implementations in child classes
    protected initDataCore(): void      { this.initData()  }
  ...
  // inheritable public functions
    ...
    public toJSON(): string { return 'JSON. yay' } // JSON.stringify() fails. write custom implementation
